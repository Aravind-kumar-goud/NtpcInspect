...
export default function IRTestDetailsScreen({ route, navigation }) {
    ...
    const loadTanDeltaCombinations = async () => {
        try {
            ...
            if (json.statusCode === 200 && Array.isArray(json.data)) {
                ...
                setTapProgress(prev => ({
                    ...
                    noOfTaps: json.data.length,  // 🔥 important
                }));
            } else {
                Alert.alert("Error", "Failed to load test combinations");
            }
        } catch (e) {
            console.log(e)
    ...
    const saveAndProceed = async () => {
        if (loading1) return;
        console.log(tapProgress.currentTap, tapProgress.noOfTaps, "\\\\\\\\\\\\\\\\\\", testCombinations.length)

        if (tapProgress.currentTap > testCombinations.length) {
    ...
    const submitImages = async () => {
        console.log(appliedVoltage, "temmmm And Voltage")
        if (!file1) {
            Alert.alert("Required", "Please capture both images");
            return;
        }
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            ...
            <View style={styles.statusContainer}>
                <Text style={styles.statusHeader}>Core IR Test Progress</Text>
                    ...
                    {testCombinations.map((tap, index) => {
                        ...
                        return (
                            <View key={tap} style={styles.tapItem}>
                                ...
                                {/* CONNECTOR */}
                                {index !== testCombinations.length - 1 && (
                                    <View
                                        style={[
                                            styles.tapLine,
                ...
                <Text style={styles.statusFooter}>
                    Completed {tapProgress.currentTap - 1} / {testCombinations.length}
                </Text>
            </View>

            <View style={styles.card}>
                <Text style={styles.title}>
                    Core IR Test Combination : {testCombinations[tapProgress.currentTap - 1]}
                </Text>
            ...
            <View style={styles.card}>
                <Text style={styles.subtitle}> CORE INSULATION RESISTANCE TEST</Text>
